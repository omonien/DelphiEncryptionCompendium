...
type
  /// <summary>
  ///   32-byte one-time Poly1305 key (r || s), typically derived from ChaCha20
  ///   block counter 0 per RFC 7539.
  /// </summary>
  ...
  private
    ...
    type
      ...
      TPoly1305CTX = packed record
        r : TStateAVXArr;
        rr : TStateAVXArr;
        k0, k1, k2, k3, k4 : T4UInt64;
        k0_5, k1_5, k2_5, k3_5, k4_5 : UInt64;
        h : TStateAVXArr;
        s0, s1 : UInt64;
      end;
      PPoly1305CTX = ^TPoly1305CTX;

      TPoly1305State = Array[0..431] of Byte;
  ...
  private
    ...
    FData : Array[0..2*POLY1305_BLOCK_SIZE - 1] of Byte;
  ...
  public
    ...
    procedure Encode(Source, Dest: PUInt8Array; Size: Integer); override;
...
begin
     ...
     g[2] := h[2] + (g[1] shr 26); g[1] := g[1] and cMask26;
     g[3] := h[3] + (g[2] shr 26); g[2] := g[2] and cMask26;
     g[4] := h[4] + (g[3] shr 26); g[3] := g[3] and cMask26;
...
begin
  ...
  // RFC 7539: one-time key is 32 bytes (r || s), produced by ChaCha20 block 0.
  if Length(InitVector) <> Length(FIV) then
    raise EDECCipherException.CreateRes(@sPoly1305InvalidKeyLength);
...
procedure TPoly1305.EnsureAADAbsorbed;
var
  ALen: Integer;
  Vec: TBytes;
begin
  ...
  if ALen > 0 then
  begin
    // Pad AAD to a 16-byte boundary before ciphertext (RFC 7539 section 2.8)
    ...
    begin
      ...
      FPolyBlkFunc(@FDataToAuthenticate[0], ALen);
  end;
...
begin
  ...
  if Size > 0 then
  begin
    // Encrypt plaintext -> ciphertext, then MAC the ciphertext
    FEncryptionMethod(Source, Dest, Size);
    FPolyBlkFunc(PByteArray(Dest), Size);
